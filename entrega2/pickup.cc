...
const std::vector<std::vector<std::vector<int>>> Pickup::sprites = {
    {
        {_,_,y,y,y,y,k,k,_,_},
        {_,y,y,y,y,y,y,k,k,_},
        {_,y,y,b,b,y,y,k,k,_},
...
bool Pickup::update(pro2::Window& window, const Finder<Platform>& platforms, const Finder<Block>& blocks) {
    if (type_ == 0) {
        pos_ += {0, coin_speed};
        despawn_timer_ --;
...
void Pickup::paint(pro2::Window& window) const {
    if (type_ == 2) {
    ...
    else paint_sprite(window, pos_, ((type_==0)? sprites[0]: sprites[1]), false);
}

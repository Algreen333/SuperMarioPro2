...
class Block {
 private:
    ...
    int block_type_; // 1 if it is a brick, 1 question mark or 2 activated block
    int has_object_; // 1 if it has a coin, 2 if it has a mushroom, nothing otherwise
 ...
 public:
    ...
    /**
     * @brief Returns what type of block it is
     * 
     * @returns
     *  - 0 if it is a 'brick block'
     * 
     *  - 1 if it is a 'question mark block'
     * 
     *  - 2 if it is an 'activated block'
    */ 
    ...
    /**
     * @brief Returns the array of the sprite for the corresponding frame of the animation
     */
    ...
    /**
     * @brief Checks collision with a block and if it needs to be activated. Returns:
     * 
     * - 0 if nothing should happen
     * 
     * - 1 if the block has to be destroyed
     * 
     * - 2 if a coin has to be spawned (without breaking the block)
     * 
     * - 3 if a mushroom has to be spawned (without breaking the block)
     */

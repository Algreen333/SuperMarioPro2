...
void Game::update_objects(pro2::Window& window) {
    ...
    // Check that Mario is not outside world limits.
    ...
    while (p != interactables_.end()) {
        ...
        // Check if a block has been brocken.
        else if (res) {
            p = interactables_.erase(p);
        }
        // Check if it has touched the finish flag.
